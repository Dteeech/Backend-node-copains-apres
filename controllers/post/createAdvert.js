...
const createAdvert = (req, res) => {
    try {
        ...
        if(id_category === 1) {
            const { housing_type, location_type, has_garage, is_furnished, surface_area, pets_allowed, wifi_available, low_mobility_friendly, has_garden, rent, number, street, city, zip, images } = req.body
            ...
            const id_advert = result.insertId

            for(let i = 0; i>=images.length; i++){
                let sqli = "INSERT INTO Picture (url, id_advert) VALUES (?)"
                pool.query(sqli, [images[i], id_advert], (err, result) => {
                if (err) {
                    console.error('Error executing SQL query:', err)
                    return res.status(500).json({ error : 'Internal server error' })
                }
